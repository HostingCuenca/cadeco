...
export default function WhatsAppFloat() {
  ...
  const handleWhatsAppClick = () => {
    const mensaje = "Hola, vengo del sitio web de CADECO, me gustaría obtener más información sobre sus productos y servicios.";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
      {/* Tooltip */}
      {showTooltip && (
        <div className="relative bg-white shadow-lg px-4 py-3 max-w-xs rounded-lg">
          <button
            onClick={() => setShowTooltip(false)}
            className="absolute -top-2 -right-2 p-1 bg-cadeco-orange text-white rounded-full hover:bg-cadeco-orange-light transition-colors"
